...
def create_ovs(ovs_name):
    ...
    dpid = _DPID_MAP.get(ovs_name, "")
    basic_cmd = 'sudo ovs-vsctl add-br {}'.format(ovs_name)
    os.system(basic_cmd)
    if dpid:
        os.system('sudo ovs-vsctl set bridge {} other-config:datapath-id={}'.format(ovs_name, dpid))
    os.system('sudo ovs-vsctl set bridge {} protocols=OpenFlow13'.format(ovs_name))
...
def create_sff(container_name, ovs_name, ip_address_1, port_1, ip_address_2, port_2):
    ...
    if not lxc_driver.create_container(container_name):
        return False
    lxc_driver.modify_configuration_bridge(container_name, '2')
    lxc_driver.add_network_interface(container_name, '2')  # add lxc.net.1.link = br2<name> for eth1
...
def create_lxc_container(container_name, ovs_name, ovs_port, ip_address, diff='5'):
    ...
    lxc_driver.container_bridge_ovs(container_name, ovs_name, str(ovs_port))
    time.sleep(5)
    if not lxc_driver.start_container(container_name):
        return False
    time.sleep(2)
