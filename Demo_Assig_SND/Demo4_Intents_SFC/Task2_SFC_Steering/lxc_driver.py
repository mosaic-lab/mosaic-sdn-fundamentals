...
def create_container(container_name):
    ...
    if not c.create("download", lxc.LXC_CREATE_QUIET, {"dist": "ubuntu",
                                                       "release": "jammy", ## change the release of OS: sudo lxc-create -n dummy -t download -- --list 2>&1 | grep ubuntu
                                                       "arch": "amd64"}):
...
def modify_configuration_bridge(container_name, diff=''):
    """
...
def add_network_interface(container_name, diff=''):
    """Append a second (or subsequent) lxc.net.N block to the container config."""
    config_path = '{}{}/config'.format(LXC_PATH, container_name)
    # Count existing lxc.net.N.type entries to determine the next index
    index = 0
    with open(config_path, 'r') as f:
        for line in f:
            if line.strip().startswith('lxc.net.') and '.type' in line:
                index += 1
    with open(config_path, 'a') as f:
        f.write('\nlxc.net.{}.type = veth\n'.format(index))
        f.write('lxc.net.{}.link = br{}{}\n'.format(index, diff, container_name))
        f.write('lxc.net.{}.flags = up\n'.format(index))
