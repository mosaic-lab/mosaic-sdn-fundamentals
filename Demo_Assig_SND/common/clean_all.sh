...
# Usage: sudo ./clean_all.sh [--onos-only] [--topo-only]
#
# Clears: LXC/LXD containers, network namespaces, OVS bridges, veth pairs,
#         Linux bridges, Mininet state, and ALL ONOS runtime state
#         (flows, intents, hosts, groups, meters, devices, network-cfg,
#          and non-core active applications).
...
# ══════════════════════════════════════════════════════════════════════════════
# PART 1 — Topology cleanup
# ══════════════════════════════════════════════════════════════════════════════
if [[ "$ONOS_ONLY" == false ]]; then

    echo "=== [1/7] Cleaning Mininet state ==="
    mn -c 2>/dev/null || true
    echo "    Done."

    echo "=== [2/7] Cleaning LXC containers ==="
    for c in $(lxc-ls 2>/dev/null); do
        echo "    Stopping/deleting LXC: $c"
        lxc-stop -n "$c" 2>/dev/null || true
        lxc-destroy -n "$c" 2>/dev/null || true
    done

    echo "=== [3/7] Cleaning LXD containers ==="
    for c in $(lxc list --format csv --columns n 2>/dev/null); do
        echo "    Stopping/deleting LXD: $c"
        lxc stop "$c" --force 2>/dev/null || true
        lxc delete "$c" --force 2>/dev/null || true
    done

    echo "=== [4/7] Cleaning namespaces, OVS, veth pairs, Linux bridges ==="
    ...
    # OVS: disconnect controllers first, then delete bridges
    for br in $(ovs-vsctl list-br 2>/dev/null); do
        echo "    Removing OVS controller on: $br"
        ovs-vsctl del-controller "$br" 2>/dev/null || true
    done
    for br in $(ovs-vsctl list-br 2>/dev/null); do
    ...
    for iface in $(ip -o link show | awk -F': ' '{print $2}'); do
        if [[ "$(ethtool -i "$iface" 2>/dev/null | grep 'driver: veth')" != "" ]]; then
            echo "    Deleting veth: $iface"
            ip link delete "$iface" 2>/dev/null || true
        fi
    done

    # Leftover Linux bridges (lxc_driver style)
    for br in $(ip -o link show type bridge | awk -F': ' '{print $2}'); do
        if [[ "$br" == br* ]]; then
            echo "    Deleting Linux bridge: $br"
    ...
    echo "=== [1-4/7] Skipping topology cleanup (--onos-only) ==="
fi

# ══════════════════════════════════════════════════════════════════════════════
# PART 2 — ONOS state cleanup
# ══════════════════════════════════════════════════════════════════════════════
if [[ "$TOPO_ONLY" == true ]]; then
    echo ""
    echo "=== Skipping ONOS cleanup (--topo-only) ==="
    echo ""
    echo "=========================================="
...
if [[ -z "$DEVICE_IDS" ]]; then
    ...
    while IFS= read -r dev_id; do
        onos_delete "/flows/${dev_id}"
        echo "    Removed flows on: $dev_id"
    done <<< "$DEVICE_IDS"
fi
...
# Devices (must come last — flows/groups already cleared)
echo "  -- Devices --"
if [[ -z "$DEVICE_IDS" ]]; then
    echo "    No devices to remove."
else
    while IFS= read -r dev_id; do
        onos_delete "/devices/${dev_id}"
        echo "    Removed device: $dev_id"
    done <<< "$DEVICE_IDS"
fi
